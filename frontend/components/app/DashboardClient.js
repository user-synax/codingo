...
export function DashboardClient({ user: initialUser, courses, initialProgress }) {
  const byLessonId = useProgressStore((s) => s.byLessonId);
  const hydrated = useProgressStore((s) => s.hydratedFromCache);
  ...
  const user = useMemo(() => {
    ...
    return {
      ...initialUser,
      ...stats,
      // preserve nested streak correctly
      streak: stats.streak ?? initialUser?.streak,
      xp: typeof stats.xp === "number" ? stats.xp : initialUser?.xp,
  ...
  const completedIds = new Set((progress ?? []).filter((p) => p.status === "completed").map((p) => String(p.lessonId)));
  ...
  const nextEntry = perCourseStatus.flatMap((c) => c.withStatus.map((x) => ({ ...x, courseTitle: c.course?.title }))).find((x) => x.status === "available") ?? null;
  const nextLesson = nextEntry?.lesson ?? null;
  const nextIndex = nextLesson ? perCourseStatus.flatMap((c) => c.withStatus.map((x) => x.lesson)).findIndex((l) => String(l._id) === String(nextLesson._id)) : -1;
  ...
  const stats = [
    { label: "XP", value: String(user?.xp ?? 0), sub: "total points earned", icon: Zap, tile: "bg-storybook-green text-eager-green" },
    { label: "Streak", value: `${user?.streak?.count ?? 0} day${(user?.streak?.count ?? 0) === 1 ? "" : "s"}`, sub: completedCount === 0 ? "Start lesson 1 today" : "keep it going", icon: Flame, tile: "bg-[#fff4e6] text-[#ff9600]" },
    { label: "Level", value: `Lv ${xpInfo.level}`, sub: `${xpInfo.have}/${xpInfo.need} XP to Lv ${xpInfo.nextLevel}`, icon: Trophy, tile: "bg-[#e6f4ff] text-spark-blue" },
    ...
    <div className="mx-auto w-full max-w-[1100px]">
      ...
      <div className="relative overflow-hidden rounded-[16px] border-2 border-faded-gray bg-paper-white p-[20px] sm:p-[28px] md:rounded-[20px]">
        ...
        <div className="relative">
          <p className="font-codingo-sans text-[13px] font-bold uppercase tracking-[0.053em] text-pencil-gray">Welcome back · {todayLine}</p>
          <h1 className="mt-1 font-codingo-sans text-[26px] font-bold leading-[1.15] text-charcoal sm:text-[32px]">Hey, {user?.username ?? "learner"} — ready to code?</h1>
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            ...
            <Link href="/app/community" className="codingo-btn-outline inline-flex items-center gap-2 rounded-[12px] border-2 border-faded-gray bg-paper-white px-6 py-3 font-codingo-sans text-[14px] font-bold leading-[1.4] text-spark-blue hover:border-spark-blue hover:bg-spark-blue/5">
        ...
        {stats.map((s) => (
          <div key={s.label} className="rounded-[16px] border-2 border-faded-gray bg-paper-white p-4 sm:p-5">
            <div className="flex items-center justify-between">
              <p className="font-codingo-sans text-[12px] font-bold uppercase tracking-[0.053em] text-pencil-gray sm:text-[13px]">{s.label}</p>
            ...
            <p className="mt-2 truncate font-codingo-sans text-[22px] font-bold leading-none text-charcoal sm:text-[28px]">{s.value}</p>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-5">
        <div className="rounded-[16px] border-2 border-charcoal bg-paper-white p-5 sm:p-6 lg:col-span-3">
          <p className="font-codingo-sans text-[12px] font-bold uppercase tracking-[0.053em] text-pencil-gray sm:text-[13px]">Up next</p>
          {nextLesson ? (
            <>
              <p className="mt-2 inline-flex items-center rounded-full bg-[#e6f4ff] px-2.5 py-0.5 font-codingo-sans text-[11px] font-bold leading-[1.4] text-spark-blue">{nextLesson.unitTitle}</p>
              {nextEntry?.courseTitle ? <p className="mt-2 inline-flex items-center rounded-full bg-charcoal px-2.5 py-0.5 font-codingo-sans text-[11px] font-bold leading-[1.4] text-paper-white ml-2">{nextEntry.courseTitle}</p> : null}
              <h2 className="mt-2 font-codingo-sans text-[20px] font-bold leading-[1.2] text-charcoal sm:text-[22px]">{nextIndex + 1}. {nextLesson.title}</h2>
              <p className="mt-1 font-codingo-sans text-[14px] font-medium leading-[1.45] text-pencil-gray">{nextLesson.description ?? "A bite-sized lesson."}</p>
              ...
              <div className="mt-4 flex flex-wrap items-center gap-3">
                <Link href={`/app/learn/${String(nextLesson._id)}`} className="codingo-btn codingo-btn-primary inline-flex items-center gap-1.5 rounded-[12px] border-2 border-eager-green bg-eager-green px-5 py-2.5 font-codingo-sans text-[13px] font-bold uppercase leading-none tracking-[0.053em] text-paper-white hover:brightness-95">
                  Start lesson
                  <ArrowRight className="h-4 w-4" strokeWidth={2.6} aria-hidden="true" />
                </Link>
                <span className="rounded-full bg-storybook-green px-3 py-1 font-codingo-sans text-[12px] font-black leading-[1.4] text-charcoal">+{nextLesson.xpReward ?? 10} XP</span>
              </div>
            </>
          ) : (
            <>
              <h2 className="mt-2 font-codingo-sans text-[20px] font-bold leading-[1.2] text-charcoal sm:text-[22px]">{totalCount ? "Course complete — be proud" : "Lessons are on the way"}</h2>
              <p className="mt-1 font-codingo-sans text-[14px] font-medium leading-[1.45] text-pencil-gray">{totalCount ? `You finished all ${totalCount} lessons. Review the path or help others in the community.` : "Course content is being prepared."}</p>
              <div className="mt-4">
                <Link href="/app/learn" className="codingo-btn codingo-btn-primary inline-flex items-center gap-1.5 rounded-[12px] border-2 border-eager-green bg-eager-green px-5 py-2.5 font-codingo-sans text-[13px] font-bold uppercase leading-none tracking-[0.053em] text-paper-white hover:brightness-95">
                  View learning path
                  <ArrowRight className="h-4 w-4" strokeWidth={2.6} aria-hidden="true" />
                </Link>
              </div>
        ...
        <div className="rounded-[16px] border-2 border-faded-gray bg-paper-white p-5 sm:p-6 lg:col-span-2">
          <div className="flex items-center justify-between">
            ...
            <span className="rounded-full bg-faded-gray/20 px-2.5 py-1 font-codingo-sans text-[11px] font-black leading-none text-pencil-gray">{week.reduce((a, w) => a + w.count, 0)} done</span>
          </div>
            ...
            {week.map((day, i) => (
              <div key={i} className="flex min-w-0 flex-1 flex-col items-center gap-1.5">
                <div className="flex h-[72px] w-full items-end justify-center">
                  <div className={`w-full max-w-[28px] rounded-full ${day.count > 0 ? "bg-eager-green" : day.isToday ? "bg-charcoal/25" : "bg-faded-gray/20"}`} style={{ height: `${day.count > 0 ? Math.max(18, Math.round((day.count / weekMax) * 100)) : 8}%` }} title={`${day.count} lesson${day.count === 1 ? "" : "s"}`} />
                </div>
                <span className={`font-codingo-sans text-[11px] font-black leading-none ${day.isToday ? "text-charcoal" : "text-pencil-gray"}`}>{day.date.toLocaleDateString("en-US", { weekday: "narrow" })}</span>
              </div>
            ))}
          </div>
          <p className="mt-3 text-center font-codingo-sans text-[12px] font-medium text-pencil-gray">{week[6].count > 0 ? "Today's lesson is done — streak safe." : "Finish a lesson today to light up the last bar."}</p>
        </div>
      </div>
      ...
      <div className="mt-4 rounded-[16px] border-2 border-faded-gray bg-paper-white p-5 sm:p-6">
        <div className="flex items-center justify-between">
          ...
          <Link href="/app/learn" className="inline-flex items-center gap-1 font-codingo-sans text-[13px] font-bold text-spark-blue hover:underline">Full path <ArrowRight className="h-3.5 w-3.5" strokeWidth={2.6} aria-hidden="true" /></Link>
        </div>
        {recent.length ? (
            ...
            {recent.map((r) => (
              <li key={String(r._id ?? r.lessonId)} className="flex items-center gap-3 py-2.5">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate font-codingo-sans text-[14px] font-bold leading-[1.3] text-charcoal">{r.lesson?.title ?? "Lesson"}</p>
                  <p className="truncate font-codingo-sans text-[12px] font-medium leading-[1.3] text-pencil-gray">{r.lesson?.unitTitle ?? ""} · {r.completedAt ? new Date(r.completedAt).toLocaleDateString("en-US", { month: "short", day: "numeric" }) : ""}</p>
                </div>
                <span className="shrink-0 rounded-full bg-faded-gray/20 px-2.5 py-1 font-codingo-sans text-[12px] font-black leading-none text-charcoal">{r.score ?? 0}%</span>
              </li>
            ))}
          </ul>
        ) : (
          <div className="mt-3 rounded-[12px] border-2 border-dashed border-faded-gray p-5 text-center">
            <p className="font-codingo-sans text-[14px] font-medium leading-[1.4] text-pencil-gray">Nothing completed yet — your finished lessons will show up here.</p>
            <Link href="/app/learn" className="mt-3 inline-flex items-center gap-1.5 font-codingo-sans text-[13px] font-bold text-spark-blue hover:underline">Browse the path <ArrowRight className="h-3.5 w-3.5" strokeWidth={2.6} aria-hidden="true" /></Link>
          </div>
        )}
      </div>
    </div>
